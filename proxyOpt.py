...
'''
Module ``proxyOpt`` provides a function ``proxyOpt.setOptions`` to control which
proxyANC  module to load, and how it is loaded, and a class ``proxyOpt.Params``
that helps users manage simulation parameters.

When proxyANC  is loaded, it checkes a few environmental variables
(``proxyOptIMIZED``, ``PROXY ALLELETYPE``, and ``PROXY DEBUG``) to determine which
proxyANC  module to load, and how to load it. More options can be set using the
``proxyOpt.setOptions`` function. For example, you can suppress the banner
message when proxyANC  is loaded and require a minimal revision of proxyANC  for
your script. proxyANC  recognize the following commandline arguments

``--optimized``
    Load the optimized version of a proxyANC  module.
...
``--gui=True|batch|False|wxPython|Tkinter``
    ...
    parameter ``gui`` for ``proxyOpt.setOptions`` for details.
...
#
# proxyOptions that will be checked when proxyANC  is loaded. This structure
...
# Optimized: command line option --optimized or environmental variable proxyOptIMIZED
if '--optimized' in sys.argv or os.getenv('proxyOptIMIZED') is not None:
    proxyOptions['Optimized'] = True

# AlleleType: from environmental variable PROXY ALLELETYPE
if os.getenv('PROXY ALLELETYPE') in ['short', 'long', 'binary']:
   proxyOptions['AlleleType'] = os.getenv('PROXY ALLELETYPE')
elif os.getenv('PROXY ALLELETYPE') is not None:
    print 'Environmental variable PROXY ALLELETYPE can only be short, long, or binary'

# Debug: from environmental variable PROXY DEBUG
if os.getenv('PROXY DEBUG') is not None:
    proxyOptions['Debug'].extend(os.getenv('PROXY DEBUG').split(','))

# GUI: from environmental variable PROXY GUI
if os.getenv('PROXY GUI') is not None:
    _gui = os.getenv('PROXY GUI')
...
if _gui in ['True', 'true', '1']:
    proxyOptions['GUI'] = True
elif _gui in ['False', 'false', '0']:
    proxyOptions['GUI'] = False
elif _gui in ['wxPython', 'Tkiner', 'batch']:
    proxyOptions['GUI'] = _gui
elif _gui is not None:
    print "Invalid value '%s' for environmental variable PROXY GUI or commandline option --gui." % _gui

def setOptions(alleleType=None, optimized=None, gui=None, quiet=None,
        debug=None, version=None, revision=None):
    '''Set options before proxyANC  is loaded to control which proxyANC  module to
    load, and how the module should be loaded.

    alleleType
        Use the standard, binary or long allele version of the proxyANC  module
        ...
        from environmental variable ``PROXY ALLELETYPE``. The standard (short)
    ...
    optimized
        ...
        if environmental variable ``proxyOptIMIZED`` is defined. The standard
    ...
    gui
        ...
        ``PROXY GUI`` or commandline option ``--gui`` for a value, and assume
        ``True`` if such an option is unavailable. If ``gui=True``, proxyANC 
        ...
        graphical toolkit to used. If ``gui='False'``, a proxyANC  script will
    ...
    quiet
        If set to ``True``, suppress the banner message when a proxyANC  module
        is loaded.

    debug
        A list of debug code (as string) that will be turned on when proxyANC 
        is loaded. If this parameter is not set, a list of comma separated
        debug code specified in environmental variable ``PROXY DEBUG``, if
        available, will be used. Note that setting ``debug=[]`` will remove
        any debug code that might have been by variable ``PROXY DEBUG``.

    version
        A version string (e.g. 1.0.0) indicating the required version number
        for the proxyANC  module to be loaded. proxyANC  will fail to load if the
        installed version is older than the required version.

    revision
        A number indicating the required revision number for the proxyANC  module
        to be loaded. proxyANC  will fail to load if the installed proxyANC  is
        older than the required revision. Please check proxyANC  ChangeLog for
        the revision number of distributed versions.
    '''
    # Optimized
    if optimized in [True, False]:
        proxyOptions['Optimized'] = optimized
    ...
    if alleleType in ['long', 'binary', 'short']:
        proxyOptions['AlleleType'] = alleleType
    ...
    if type(version) == type(''):
        try:
            ...
            print 'Invalid version string %s' % proxyOptions['Version']
    ...
    if type(revision) == type(1):
        proxyOptions['Revision'] = revision
    ...
    def func(val):
        ...
        if size is not None:
            if isinstance(size, (int, long)):
                ...
                raise exceptions.ValueError('Invalid size specification for proxyOpt.valueIsList')
        return True
    return func
...
options:
  ...
  --optimized
        Run the script using an optimized proxyANC  module.
...
def param(**kwargs):
    ...
    ``proxyOpt.Params`` for allowed keyword arguments and their meanings.'''
    return kwargs

class Params:
    '''
    class Params provides a uniform interface for proxyANC  scripts to handle
    ...
    def addOption(self, pos=-1, **kwargs):
        ...
        for key in kwargs:
            if key in allowed_keys:
                ...
                if key == 'allowedTypes' and isinstance(kwargs[key], types.TypeType):
                    ...
                    # must be a list.
                    opt[key] = kwargs[key]
                if key == 'useDefault' and 'DBG_COMPATIBILITY' in proxyOptions['Debug']:
                    print >> sys.stderr, 'WARNING: useDefault is obsolete and might be removed from a future version of PROXYANC'
            else:
        ...
        # 
        # proxyANC  1.0.3 disallow invalid default value in parameter specification
    ...
    def termGetParam(self, params=[]):
        ...
        for opt in self.options:
            ...
            if len(params) == 0 and (
                ...
                ('DBG_BATCHTESTING' in proxyOptions['Debug']) or \
